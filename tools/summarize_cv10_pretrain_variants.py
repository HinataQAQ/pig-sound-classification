...
def parse_model(exp):
    ...
    if "_mctafd_no_se_gated_" in s:
        return "mctafd_no_se_gated_pretrain"

    if "_logmel_" in s:
        return "logmel_pretrain"
...
for summary_path in REPORTS.glob("cv10_pretrain_no_feeding*/summary.json"):
    ...
    rows.append({
        ...
        "load_encoder_only": m.get("load_encoder_only"),
...
if len(df) == 0:
    raise RuntimeError("No cv10 pretrain summary.json found.")
...
runs_out = REPORTS / "cv10_pretrain_variant_runs.csv"
...
summary_out = REPORTS / "cv10_pretrain_variant_summary.csv"
summary.to_csv(summary_out, index=False, encoding="utf-8-sig")

print("\n[CV10 PRETRAIN RUNS]")
print(df.to_string(index=False))

print("\n[CV10 PRETRAIN SUMMARY]")
